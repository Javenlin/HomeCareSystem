...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String KEY_PIC_CODE        = "code";
    ...
    private static final String CREATE_PILL_TABLE =
            "create table " + PILL_TABLE + "("
                    ...
                    + KEY_PIC_URI  + " text not null,"
                    + KEY_PIC_CODE  + " text not null,"
    ...
    public void onDestory(SQLiteDatabase db) {
        db.execSQL("DROP TABLE IF EXISTS " + PILL_TABLE);
        db.execSQL("DROP TABLE IF EXISTS " + ALARM_TABLE);
        db.execSQL("DROP TABLE IF EXISTS " + PILL_ALARM_LINKS);
        db.execSQL("DROP TABLE IF EXISTS " + HISTORIES_TABLE);
...
// ############################## create methods ###################################### //
    ...
    public long createPill(Pill pill) {
        ...
        values.put(KEY_PIC_URI, pill.getPillUri());
        values.put(KEY_PIC_CODE, pill.getPillCode());
...
// ############################# get methods ####################################### //
    ...
    public Pill getPillByName(String pillName) {
        ...
        if (c.moveToFirst() && c.getCount() >= 1) {
            pill.setPillUri(c.getString(c.getColumnIndex(KEY_PIC_URI)));
            pill.setPillCode(c.getString(c.getColumnIndex(KEY_PIC_CODE)));
...
//     * @return returns a pill model object
//     */
//    public Pill getPillById(long pill_id) throws URISyntaxException {
//        SQLiteDatabase db = this.getReadableDatabase();
//
//        String dbPill = "select * from "
//                + PILL_TABLE        + " where "
